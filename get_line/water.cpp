...
bool input_assist(Mat im,map<string, string> main_ini, vector<assist_information> & assist_files)
{
	...
	for(int i=0;i<temp_assist_files.size();++i) {
		...
		find_object(im, d_image, im2, base_rect, contours, temp_result_point, temp_score);
		if (temp_score > score) {
			score = temp_score;
			result_point = temp_result_point;
			result = draw_contours(im, contours, result_point, 3);
		}
		if (assist_files.size() == temp.ref_index+1) {
			if (score < assist_files[temp.ref_index].correct_score)
				continue;
		}
		else {
		...
		if (assist_files.size() == temp.ref_index + 1)
			assist_files[temp.ref_index] = temp;
		else if (assist_files.size() < temp.ref_index + 1)
			assist_files.push_back(temp);
		else
			return false;
	}
...
void get_parrallel_lines(assist_information &assist_file)
{
	...
	Vec4f line_para;
	fitLine(Points, line_para, CV_DIST_L2, 0, 1e-2, 1e-2);

	//Mat im = assist_file.assist_image.clone();
	//line(im, x1,x2, Scalar(0, 255, 0), 2);
	return;
}
