...
bool input_assist(Mat im,map<string, string> main_ini, vector<assist_information> & assist_files, bool assist_flag)
{
	...
	for(int i=0;i<temp_assist_files.size();++i) {
		...
		find_object(im, d_image, im2, base_rect, contours, result_point, score,temp.sub_roi);
		//
		result = draw_contours(im, contours, result_point, 1);
...
void compute_water_area(Mat im, vector<assist_information> &assist_files, string ref_name)
{
	for (auto &assist_file : assist_files) {
		...
		if(assist_file.left_right_no_water)
			assist_file.water_line = get_water_line_seg(image_rotate, assist_file.length, assist_file.length,0.9);
		else {
			assist_file.water_line = get_water_line_seg(image_rotate.colRange(image_rotate.cols/3, image_rotate.cols*2 / 3), assist_file.length,0.9);
		}
...
void find_object(Mat search_image, vector<Mat> merge_im,
	...
	Point & result_point, float& score, vector<double> roi)
{
	// 图像转灰度处理
	...
	Rect searchArea;
	if (roi.size()<4)
		searchArea = Rect(0, 0, gray_image.cols - template_gray_image.cols,
			gray_image.rows - template_gray_image.rows);
	else {

		searchArea = Rect(roi[0]-roi[2]/2, roi[1] - roi[3] / 2, roi[2],roi[3]);
	}
	...
	for (int i = searchArea.y; i < searchArea.y + searchArea.height ; i++)
	{
		for (int j = searchArea.x; j < searchArea.x + searchArea.width; j++)
		{
			...
			for (int feaIdx = 0; feaIdx < contours.size(); feaIdx++)
			{
				...
				n = n + 1;
				if (n < contours.size()*0.2)
					continue;
				if (similarity < min_score*n)
				{
					break;
				}
